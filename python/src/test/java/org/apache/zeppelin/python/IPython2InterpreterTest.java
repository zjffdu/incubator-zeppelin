...
public class IPython2InterpreterTest {

  private static final Logger LOGGER = LoggerFactory.getLogger(IPython2InterpreterTest.class);
  ...
  public void testPython2() {
    ...
    if (!interpreter.getPythonVersion().equals("2")) {
      LOGGER.warn("Skip test because it is not python2");
      return;
    }
    ...
    assertEquals(InterpreterResult.Code.SUCCESS, result.code());
    assertTrue(result.message().get(0).getData().contains("Return a list containing an arithmetic"));
