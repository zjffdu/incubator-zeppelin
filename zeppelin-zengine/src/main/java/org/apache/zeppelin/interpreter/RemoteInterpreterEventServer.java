...
import org.apache.zeppelin.interpreter.thrift.AngularObjectId;
...
public class RemoteInterpreterEventServer implements RemoteInterpreterEventService.Iface {
  ...
  public RemoteInterpreterEventServer(ZeppelinConfiguration zConf,
    ...
    this.portRange = zConf.getCallbackPortRange();
  ...
  public void callback(CallbackInfo callbackInfo) throws TException {
    InterpreterGroup interpreterGroup =
        interpreterSettingManager.getInterpreterGroupById(callbackInfo.getInterpreterGroupId());
    if (interpreterGroup == null) {
      LOGGER.warn("No such interpreterGroup: " + callbackInfo.getInterpreterGroupId());
      return;
    }
    ...
    if (interpreterProcess == null) {
      LOGGER.warn("Interpreter process does not existed yet for InterpreterGroup: " +
          callbackInfo.getInterpreterGroupId());
    }
    ((RemoteInterpreterManagedProcess) interpreterProcess)
        .processStarted(callbackInfo.port, callbackInfo.host);
  }
