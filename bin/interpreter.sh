...
if [[ -n "${SPARK_SUBMIT}" ]]; then
  ...
  IFS=' ' read -r -a ZEPPELIN_FLINK_YARN_APPLICATION_CONF_ARRAY <<< "${ZEPPELIN_FLINK_YARN_APPLICATION_CONF_ARRAY}"
...
elif [[ "${ZEPPELIN_FLINK_K8S_APPLICATION}" == "true" ]]; then
  IFS=' ' read -r -a ZEPPELIN_K8S_YARN_APPLICATION_CONF_ARRAY <<< "${ZEPPELIN_FLINK_K8S_APPLICATION_CONF_ARRAY}"
  INTERPRETER_RUN_COMMAND+=("${FLINK_HOME}/bin/flink" "run-application" "-c" "${ZEPPELIN_SERVER}" "-t" "kubernetes-application" "${ZEPPELIN_FLINK_K8S_APPLICATION_CONF_ARRAY[@]}" "${FLINK_APP_JAR}" "${CALLBACK_HOST}" "${PORT}" "${INTP_GROUP_ID}" "${INTP_PORT}")
else
  IFS=' ' read -r -a JAVA_INTP_OPTS_ARRAY <<< "${JAVA_INTP_OPTS}"
