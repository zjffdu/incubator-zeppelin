...
public class RemoteInterpreterServer extends Thread
    ...
    if (null != intpEventServerHost && !isTest) {
      new Thread(new Runnable() {
        ...
        public void run() {
          ...
          if (!interrupted) {
            CallbackInfo callbackInfo = new CallbackInfo(host, port, interpreterGroupId);
            try {
              intpEventServiceClient.callback(callbackInfo);
            } catch (TException e) {
  ...
  public static void main(String[] args)
    ...
    RemoteInterpreterServer remoteInterpreterServer =
        new RemoteInterpreterServer(callbackHost, port, interpreterGroupId, portRange);
